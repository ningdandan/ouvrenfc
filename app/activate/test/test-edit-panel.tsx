...
export function TestEditPanel({ handle, user, onSaved, onCancel }: Props) {
  ...
  const handleSave = () => {
    ...
    window.setTimeout(() => {
      setSaveToast(true);
      window.setTimeout(() => setSaveToast(false), 800);
      setIsSaving(false);
      onSaved({ ...user, spaceName, socialLinks: normalizedLinks, theme });
    }, 220);
  };

  const isReady = !isSaving;
    ...
    <BackroomPanelSection>
      ...
      <div className="relative z-10 flex flex-col flex-1 overflow-y-auto">
        ...
        <div className="flex flex-col flex-1 justify-center gap-6 pb-[max(1.5rem,env(safe-area-inset-bottom))]">
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        ...
                        />
                        {!selected && <div className="absolute inset-0 bg-black/60" />}
                      </div>
                      <span
          ...
          {/* Save button */}
          <BackroomSubmitButton isReady={isReady} type="button" onClick={handleSave}>
